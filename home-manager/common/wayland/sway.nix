  ...
  wayland.windowManager.sway = {
    ...
    config = rec {
      ...
      window = {
        ...
        commands = [
          {
            command = "border none, floating enable, sticky enable, move position cursor";
            criteria = {
      ...
      keybindings =
        ...
        lib.mkOptionDefault {
          # Emoji Picker
          "$modalt+Space" = ''exec /usr/bin/env BEMOJI_ECHO_NEWLINE=false BEMOJI_PICKER_CMD="${pkgs.fuzzel}/bin/fuzzel --dmenu" ${pkgs.bemoji}/bin/bemoji'';
          "Mod1+Mod4+Control+Shift+Space" = ''exec /usr/bin/env BEMOJI_ECHO_NEWLINE=false BEMOJI_PICKER_CMD="${pkgs.fuzzel}/bin/fuzzel --dmenu" ${pkgs.bemoji}/bin/bemoji'';
          # Window Switcher
    ...
    settings = [
      ...
      }
      {
        ...
        profile.outputs = [
          ...
          {
            criteria = "Hewlett Packard HP 23xw 6CM7072CGL";
            position = "1706,764";
            scale = 1.0;
            status = "enable";
          }
          {
            criteria = "Dell Inc. DELL P2414H 524N343R7N8L";
            position = "3626,0";
            transform = "90";
            scale = 1.0;
            status = "enable";
